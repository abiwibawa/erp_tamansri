<script>
	...
	$(function() {
		...
		$.ajax({
			...
			success: function(response){
				...
				$("#ppn").val(<?=$tampilpenerima->ppn?>);
				$("#total_harga").val(response.subtotal+(<?=$tampilpenerima->ppn?>));
				$("tbody").html("");
		...
		$("#btn_tambah_pembelian").click(function(){
			...
			if(id_suplier==""){
				...
				if($("#nama_barang").val()==""){
					$(".modal .modal-dialog .modal-header .modal-title").html("Terjadi Kesalahan");
					$(".modal .modal-dialog .modal-body").html("Barang Masih Kosong");
					$("#modal_confirm").modal("show"); 
				}else if($("#kuantitas").val()==""){
					$(".modal .modal-dialog .modal-header .modal-title").html("Terjadi Kesalahan");
					$(".modal .modal-dialog .modal-body").html("Quantity Masih Kosong");
					$("#modal_confirm").modal("show"); 
				}else if($("#harga").val()==""){
					$(".modal .modal-dialog .modal-header .modal-title").html("Terjadi Kesalahan");
					$(".modal .modal-dialog .modal-body").html("Harga Masih Kosong");
					$("#modal_confirm").modal("show"); 
				}else{
							...
							success: function(response){
								...
								$.ajax({
									...
									success: function(response){
										...
										$("#total_harga").val(response.subtotal+(response.subtotal/10));
										$("tbody").html("");
										$("tbody").append(response.vtabel);
									}
								});
			...
			if(id_suplier==""){
				...
				if($("#id_pemesanan_h").val()==""){
					$.ajax({
						...
						dataType: "html",
						url : "<?=base_url('pembelian_pemesanan/simpan')?>",
						data: {id_suplier:id_suplier,id_ttd:id_ttd,tanggalpemesanan:tanggalpemesanan,tanggalpengiriman:tanggalpengiriman,syarat_pembayaran:syarat_pembayaran,keterangan:keterangan,id_pemesanan_h:$("#id_pemesanan_h").val(),ppn:$("#ppn").val()},
						success: function(response){
							$("#id_pemesanan_h").val(response);
					...
					$.ajax({
						...
						data: {id_suplier:id_suplier,id_ttd:id_ttd,tanggalpemesanan:tanggalpemesanan,tanggalpengiriman:tanggalpengiriman,syarat_pembayaran:syarat_pembayaran,keterangan:keterangan,id_pemesanan_h:id_pemesanan_h,id_barang:id_barang,satuan:satuan,kuantitas:kuantitas,harga:harga,ppn:$("#ppn").val()},
						success: function(response){
							$(".modal .modal-dialog .modal-header .modal-title").html("Sukses Simpan Laporan Barang");
							$(".modal .modal-dialog .modal-body").html("Input pemesanan barang berhasil tersimpan dan laporan bisa dicetak");
							$("#modal_success").modal("show"); 
	...
	$(document).ajaxComplete(function () {
		$(".hapus_tambah_barang").click(function(){
			if(confirm("Yakin akan hapus barang ini?")){
				var id_pemesanan_h=$("#id_pemesanan_h").val();
...
<div class="row">             
			...
			<div class="content">
				<div class="header">
					...
					<div class="side pull-right">
						<a href="<?=base_url("pembelian_lappemesanan")?>" class=" btn btn-warning">
							<i class="icon-save"></i>&nbsp;&nbsp;kembali
						</a>
	...
	<input type="hidden" name="id_pemesanan_h" id="id_pemesanan_h" value="<?=$id_pemesanan_h?>">
	<input type="hidden" name="id_ttd" id="id_ttd" value="<?=$tampilpenerima->id_ttd?>">
	<div class="col-md-9">
		<div class="block block-fill-white">
			...
			<div class="content controls">
				<div class="form-row">
					...
					<div class="col-md-3"><?=form_input('kode_suplier',$tampilpenerima->kode_suplier,'onclick="popupcarisuplier()" class="form-control" id="kode_suplier" readonly')?></div>
					<div class="col-md-1"><button type="button" id="btn_cari_custom" class="btn btn-success" onclick="popupcarisuplier()">cari</button></div>
					<input type="hidden" name="id_suplier" id="id_suplier" value="<?=$tampilpenerima->id_suplier?>">
					<div class="col-md-2">Tanggal Pemesanan</div>
					<div class="col-md-4"> 
						<div class="input-group">
							<div class="input-group-addon"><span class="icon-calendar-empty"></span></div>
							<?=form_input('tanggalpemesanan',date( 'm/d/Y',strtotime($tampilpenerima->tanggal_pemesanan)),'class="datepicker form-control" id="tanggalpemesanan" ')?>
						</div>
					</div>
				</div>
				...
				<div class="form-row">
					...
					<div class="col-md-4"><?=form_input('nama_suplier',$tampilpenerima->nama,'class="form-control" id="nama_suplier" readonly="readonly"')?></div>
					
					<div class="col-md-2">Tanggal Pengiriman</div>
					<div class="col-md-4"> 
						<div class="input-group">
							<div class="input-group-addon"><span class="icon-calendar-empty"></span></div>
							<?=form_input('tanggalpengiriman',date( 'm/d/Y',strtotime($tampilpenerima->tanggal_pengiriman)),'class="datepicker form-control" id="tanggalpengiriman" ')?>
						</div>
					</div>
				...
				<div class="form-row">
					...
					<div class="col-md-4"><?=form_input('telp_suplier',$tampilpenerima->telpon,'class="form-control" id="telp_suplier" readonly="readonly"')?></div>
						
					<div class="col-md-2">Syarat Pembayaran</div>
					<div class="col-md-4"><textarea name="syarat_pembayaran" id="syarat_pembayaran" class="form-control"><?=$tampilpenerima->syarat_pembayaran?></textarea></div>
				</div>
				
				<div class="form-row">
					<div class="col-md-2">Kota Suplier</div>
					<div class="col-md-4"><?=form_input('kota_suplier',$tampilpenerima->kota,'class="form-control" id="kota_suplier" readonly="readonly"')?></div>
					
					<div class="col-md-2">Keterangan</div>
					<div class="col-md-4"><textarea name="keterangan" id="keterangan" class="form-control"><?=$tampilpenerima->keterangan?></textarea></div>
				</div>
				
				<div class="form-row">
					<div class="col-md-2">Alamat Suplier</div>
					<div class="col-md-4"><?=form_input('alamat_suplier',$tampilpenerima->alamat,'class="form-control" id="alamat_suplier" readonly="readonly"')?></div>
					
					<div class="col-md-2">Tanda Tangan Surat</div>
					<div class="col-md-3"><?=form_input('tanda_tangan_surat',$tampilpenerima->nama_ttd,' class="form-control" id="tanda_tangan_surat" readonly')?></div>
		...
		<div class="block block-fill-white">
			...
			<div class="content controls">
				...
				<div class="form-row">
					...
					<div class="col-md-7"><?=form_input('ppn','','class="form-control" id="ppn" readonly="readonly" ')?></div>
				</div>
				<div class="form-row">
